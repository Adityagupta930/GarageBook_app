...
export async function GET(req: NextRequest) {
  ...
  if (type === 'daily') {
    return Response.json(db.prepare(`
  ...
  const pending     = db.prepare('SELECT SUM(amount) as total FROM sales WHERE payment = "udhaar" AND udhaar_paid = 0').get() as { total: number };

  return Response.json({ totalSales, cashSales, onlineSales, creditSales, profit, totalItems, pendingCredit: pending.total || 0 });
}
